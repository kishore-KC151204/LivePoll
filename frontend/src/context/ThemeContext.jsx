...
export function ThemeProvider({children}){
  ...
  useEffect(()=>{
    ...
    root.style.setProperty('--accent-soft',`color-mix(in srgb, ${p.accent} 12%, transparent)`)
    root.style.setProperty('--accent-border',`color-mix(in srgb, ${p.accent} 42%, transparent)`)
    root.style.setProperty('--accent-glow',`color-mix(in srgb, ${p.accent} 28%, transparent)`)
  },[accent])
